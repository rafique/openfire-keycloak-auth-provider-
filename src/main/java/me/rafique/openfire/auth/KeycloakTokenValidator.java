...
public class KeycloakTokenValidator {
	...
	private static Logger logger = LoggerFactory.getLogger(KeycloakTokenValidator.class);
	private JiveProperties jiveProperties;

	public KeycloakTokenValidator(JiveProperties jiveProperties) {
	...
	public JwtClaims verifyClaims(String token, Key key) throws InvalidJwtException {
		...
		JwtConsumer secondPassJwtConsumer = new JwtConsumerBuilder()
				...
				.setExpectedAudience("sandbox-app").setJwsAlgorithmConstraints(algorithmConstraints).build();
		...
		logger.info("verified claims: {}", claims);
		return claims;
	}
